...
export class QuicConnection implements CoreConnection {
  ...
  async close(_reason?: KpsReason): Promise<void> {
    ...
    // bare `null` — un-introspectable, and crashes the process (kps#3). Error
    // each still-open half with a proper KPS 'closed' reason first: the
    // read/write still rejects (a local close is not a FIN), but with a real
    // coded Error instead of null.
